...
<div class="space-y-8 max-w-7xl mx-auto">
    ...
    <!-- Header Banner Profil Kandidat -->
    <div class="relative overflow-hidden bg-gradient-to-r from-blue-600 to-indigo-600 rounded-3xl p-8 text-white shadow-xl border border-blue-500">
        <div class="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div class="space-y-2">
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-blue-400/30 text-blue-50 border border-blue-400/30 backdrop-blur-md">
                    <svg class="w-3.5 h-3.5 text-blue-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('student.vision') }}" class="px-5 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl text-sm font-semibold transition border border-white/20 backdrop-blur-md flex items-center gap-2">
        ...
        <div class="absolute top-0 right-0 -mt-12 -mr-12 w-64 h-64 bg-blue-400/20 rounded-full blur-3xl pointer-events-none"></div>
    </div>

    <!-- Candidate List Showcase Grid -->
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        @foreach($candidates as $candidate)
        <div class="bg-white rounded-3xl shadow-sm border border-blue-100 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col justify-between group">
            
            <div>
                <!-- Card Header with Badge -->
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <span class="w-12 h-12 rounded-2xl bg-white text-blue-700 flex items-center justify-center text-xl font-black shadow-lg shadow-blue-500/30">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="relative group/photo overflow-hidden rounded-2xl border-2 border-blue-400/40 bg-blue-900 aspect-[3/4] shadow-sm">
                            @if($candidate->foto_ketua)
                                <img src="{{ asset('storage/'.$candidate->foto_ketua) }}" alt="{{ $candidate->ketua }}" class="w-full h-full object-cover transition-transform duration-500 group-hover/photo:scale-105">
                            @else
                                <div class="w-full h-full bg-gradient-to-br from-blue-800 to-indigo-900 flex flex-col items-center justify-center text-white p-3 text-center">
                                    <svg class="w-12 h-12 opacity-50 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                    <span class="text-xs font-medium text-blue-200">Foto Ketua</span>
                                </div>
                            @endif
                            
                            <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-blue-950 via-blue-900/60 to-transparent p-3 text-white">
                        ...
                        <div class="relative group/photo overflow-hidden rounded-2xl border-2 border-blue-400/40 bg-indigo-900 aspect-[3/4] shadow-sm">
                            @if($candidate->foto_wakil)
                                <img src="{{ asset('storage/'.$candidate->foto_wakil) }}" alt="{{ $candidate->wakil }}" class="w-full h-full object-cover transition-transform duration-500 group-hover/photo:scale-105">
                            @else
                                <div class="w-full h-full bg-gradient-to-br from-indigo-900 to-blue-800 flex flex-col items-center justify-center text-white p-3 text-center">
                                    <svg class="w-12 h-12 opacity-50 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
            ...
            <div class="p-6 pt-0 flex gap-3">
                <a href="{{ route('student.vision') }}" class="flex-1 text-center py-3 px-4 rounded-xl border border-blue-200 text-blue-700 hover:bg-blue-50 font-bold text-xs transition">
                    Visi & Misi Lengkap
                </a>
                <a href="{{ route('vote.index') }}" class="flex-1 text-center py-3 px-4 rounded-xl bg-blue-600 hover:bg-indigo-700 text-white font-extrabold text-xs shadow-md transition flex items-center justify-center gap-1.5">
                    <span>Menu Bilik Voting</span>
                    <svg class="w-3.5 h-3.5 text-blue-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                </a>
            </div>
